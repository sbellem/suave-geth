...
type SuaveRuntime interface {
	...
	genQuote(input1 uint64, input2 uint64) (uint64, error)
...
var addrList = []common.Address{
	buildEthBlockAddr, confidentialInputsAddr, confidentialRetrieveAddr, confidentialStoreAddr, ethcallAddr, extractHintAddr, fetchBidsAddr, fillMevShareBundleAddr, genQuoteAddr, newBidAddr, signEthTransactionAddr, simulateBundleAddr, submitBundleJsonRPCAddr, submitEthBlockBidToRelayAddr,
}
...
func (b *SuaveRuntimeAdapter) genQuote(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["genQuote"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if quote, err = b.impl.genQuote(input1, input2); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["genQuote"].Outputs.Pack(quote)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
