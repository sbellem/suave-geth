...
type SuaveRuntime interface {
	...
	getAttestationVerificationReport(input1 uint64, input2 uint64) (types.IASResponseBody, types.IASResponseHeader, error)
...
var addrList = []common.Address{
	buildEthBlockAddr, confidentialInputsAddr, confidentialRetrieveAddr, confidentialStoreAddr, ethcallAddr, extractHintAddr, fetchBidsAddr, fillMevShareBundleAddr, getAttestationVerificationReportAddr, newBidAddr, signEthTransactionAddr, simulateBundleAddr, submitBundleJsonRPCAddr, submitEthBlockBidToRelayAddr,
}
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case getAttestationVerificationReportAddr:
		return b.getAttestationVerificationReport(input)
...
func (b *SuaveRuntimeAdapter) getAttestationVerificationReport(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["getAttestationVerificationReport"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	input1 = unpacked[0].(uint64)
	input2 = unpacked[1].(uint64)
	...
	if iasResponseBody, iasResponseHeader, err = b.impl.getAttestationVerificationReport(input1, input2); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["getAttestationVerificationReport"].Outputs.Pack(iasResponseBody, iasResponseHeader)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
